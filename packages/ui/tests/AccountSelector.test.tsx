...
describe('AccountSelector', () => {
  it('renders nothing for a single account (no switcher needed)', () => {
    const { container } = render(
      <AccountSelector LinkComponent={FakeLink} accounts={[ACCOUNTS[0] as AccountSelectorItem]} activeAccountId="acc-1" />,
    );
    expect(container).toBeEmptyDOMElement();
  });

  it('lists every account with an explicit navigation link, never a silent switch', () => {
    render(<AccountSelector LinkComponent={FakeLink} accounts={ACCOUNTS} activeAccountId="acc-1" />);
  ...
  it('marks the active account with aria-current', () => {
    render(<AccountSelector LinkComponent={FakeLink} accounts={ACCOUNTS} activeAccountId="acc-2" />);
