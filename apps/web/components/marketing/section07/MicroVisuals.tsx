...
export function MicroSparkline({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height} aria-hidden="true" className="mt-1.5">
      <motion.polyline
        points={line}
        fill="none"
    ...
    <div className="mt-1.5 flex h-[22px] items-end gap-[3px]" aria-hidden="true">
      {bars.map((value, index) => (
        <motion.span
          ...
          style={{ height: `${Math.max(14, (value / max) * 100)}%`, opacity: 0.35 + (value / max) * 0.45 }}
          initial={reduced ? false : { scaleY: 0 }}
          animate={{ scaleY: 1 }}
          transition={{ duration: reduced ? 0 : 0.32, delay: reduced ? 0 : index * 0.05, ease: [0.22, 1, 0.36, 1] }}
        />
      ))}
    </div>
  );
    ...
    <div className="mt-2 flex items-center gap-1" aria-hidden="true">
      {weights.map((weight, index) => (
        <motion.span
          ...
          animate={{ scale: 1 }}
          transition={{ duration: reduced ? 0 : 0.22, delay: reduced ? 0 : index * 0.035, ease: [0.22, 1, 0.36, 1] }}
        />
      ))}
    </div>
  );
...
export function ExpectancyRail({
  positive,
  reduced,
}: {
  positive: boolean;
...
export function ProfitFactorRatio({
  value,
  reduced,
}: {
  value: number;
